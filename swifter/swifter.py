...
class SeriesAccessor(_SwifterObject):
        ...
        try:
            ...
            with suppress_stdout_stderr_logging():
                ...
                self._validate_apply(
                    tmp_df.equals(meta),
                    error_message=(
                        "Dask map-partitions sample does not match pandas apply sample."
                    ),
                )
    ...
    def apply(self, func, convert_dtype=True, args=(), **kwds):
        ...
        allow_dask_processing = (
            True if self._allow_dask_on_strings else (sample.dtype != "object")
        )
        ...
        try:  # try to vectorize
            ...
            est_apply_duration = (
                sample_proc_est / self._SAMPLE_SIZE * self._obj.shape[0]
            )
...
class DataFrameAccessor(_SwifterObject):
    ...
    def apply(self, func, axis=0, raw=False, result_type=None, args=(), **kwds):
        ...
        allow_dask_processing = (
            True
            if self._allow_dask_on_strings
            else ("object" not in sample.dtypes.values)
        )
...
class Rolling(Transformation):
    def __init__(
        ...
        **kwds,
        ...
        self._obj_dd = self._obj_dd.rolling(
            **{k: v for k, v in kwds.items() if k not in ["on", "closed"]}
        )
    ...
    def _dask_apply(self, func, *args, **kwds):
        try:
            ...
            with suppress_stdout_stderr_logging():
                ...
                self._validate_apply(
                    tmp_df.equals(
                        self._comparison_pd.rolling(**self._rolling_kwds).apply(
                            func, *args, **kwds
                        )
                    ),
                    error_message=(
                        "Dask rolling apply sample does not match "
                        "pandas rolling apply sample."
                    ),
                )
            if self._progress_bar:
                with TQDMDaskProgressBar(desc=self._progress_bar_desc or "Dask Apply"):
                    return self._obj_dd.apply(func, *args, **kwds).compute(
                        scheduler=self._scheduler
                    )
            else:
                return self._obj_dd.apply(func, *args, **kwds).compute(
            ...
            with suppress_stdout_stderr_logging():
                tmp_df = (
                    ...
                    .resample(
                        **{
                            k: v
                            for k, v in self._resampler_kwds.items()
                            if k in ["rule", "closed", "label"]
                        }
                    )
                    .agg(func, *args, **kwds)
                    .compute(scheduler=self._scheduler)
                ...
                self._validate_apply(
                    tmp_df.equals(
                        self._comparison_pd.resample(**self._resampler_kwds).apply(
                            func, *args, **kwds
                        )
                    ),
                    error_message=(
                        "Dask resampler apply sample does not match "
                        "pandas resampler apply sample."
                    ),
                )

            if self._progress_bar:
                with TQDMDaskProgressBar(desc=self._progress_bar_desc or "Dask Apply"):
                    return self._obj_dd.agg(func, *args, **kwds).compute(
                        scheduler=self._scheduler
                    )
            else:
                return self._obj_dd.agg(func, *args, **kwds).compute(
